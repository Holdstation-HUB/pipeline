...
func newScheduler(p *Pipeline, run *Run, vars Vars, lggr *zap.Logger) *scheduler {
	...
	for id, task := range p.Tasks {
		...
		lggr.Info("scheduling task run", zap.String("dot_id", task.DotID()), zap.Uint("attempts", run.attempts))
...
func (s *scheduler) reconstructResults() {
	...
	for _, r := range s.run.PipelineTaskRuns {
		...
		if err != nil {
			s.logger.Error("Vars.Set error: %v", zap.Error(err))
		}
		...
		if result.Attempts < uint(result.Task.TaskRetries()) && result.Result.Error != nil {
			...
			go func(vars Vars) {
				...
				case <-time.After(backoff.ForAttempt(float64(result.Attempts - 1))): // we subtract 1 because backoff 0-indexes
					...
					s.logger.Info("scheduling task run", zap.String("dot_id", run.task.DotID()), zap.Uint("attempts", run.attempts))
					s.taskCh <- run
				}
...
func (s *scheduler) report(ctx context.Context, result TaskRunResult) {
	...
	case <-ctx.Done():
		s.logger.Error("pipeline.scheduler: discarding result; report context timed out", zap.Any("result", result), zap.Error(ctx.Err()))
	}
}
